...
if __name__ == '__main__':
    ...
    # FIXME: improve this
    if isinstance(opt.compression_type, list):
        c = ''
        for i in opt.compression_type:
            c += i
        opt.store_name = '_'.join([opt.dataset, opt.model, c,
    ...
    if opt.resume_path and 'kd' not in opt.compression_type:
    ...
    if not opt.no_train:
        ...
        if 'ep' in opt.compression_type:
            ...
            print("pruning zero parameter count:", par1)

        if opt.compress:
            if 'qat' in opt.compression_type or 'kd' in opt.compression_type:
                compression_scheduler = distiller.CompressionScheduler(model)
                if 'qat' in opt.compression_type:
    ...
    if opt.compress and 'kd' in opt.compression_type:
    ...
    for i in range(opt.begin_epoch, opt.begin_epoch + opt.n_epochs):
        ...
        if opt.compress and 'qat' in opt.compression_type:
    ...
    if 'ptq' in opt.compression_type:
    ...
    if 'ep' in opt.compression_type:
        ...
        print("fine-tuned zero parameter count:", par)
    ...
    if 'ep' in opt.compression_type:
        print("starting pruned zero parameter count", par1)
        print("fine-tuned zero parameter count:", par2)
